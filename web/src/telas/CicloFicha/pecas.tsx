...
// (≥20 vermelho, ≥10 âmbar) pintando o KPI de "% zona crítica" com uma régua
// inventada, sem relação com o corte em vigor. Como a `toneMedia`, não foi
// substituída por outra função de cor — o KPI é MAGNITUDE e quem carrega a
// leitura é o gráfico abaixo dele (R7).
...
export function Evolucao({ stats }: { stats: EstatisticasCiclo }) {
...
export function Conjunta({ stats }: { stats: EstatisticasCiclo }) {
...
export function PorMateria({ recortes }: { recortes: RecorteMateria[] }) {
...
export function TabelaSimuladosDoCiclo({ simulados }: { simulados: Simulado[] }) {
  const navegar = useNavigate();
  return (
    <div className="section">
...
export function Avancado({ stats }: { stats: EstatisticasCiclo }) {
  return (
  ...
  return (
    <div className="ciclo-avancado__grupos">
      ...
      <Grupo titulo="Taxas">
        ...
        <MiniCard rotulo="% zona crítica" valor={`${fmtPct(stats.pctZonaCritica)}%`} />
 ...
 */
export function rotuloDaPeca(svg: SVGSVGElement): string {
...
export function tituloDoGrafico(svg: SVGSVGElement): string {
 ...
 */
export function textoDoCabecalho(titulo: Element | null | undefined): string {
  if (!titulo) return '';
...
export function secaoDoGrafico(svg: SVGSVGElement): string {
