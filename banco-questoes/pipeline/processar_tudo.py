"""
processar_tudo.py — Roda o pipeline para todos os PDFs em pdfs_originais/
nas matérias indicadas, pulando provas já processadas.

Uso:
    python pipeline/processar_tudo.py
    python pipeline/processar_tudo.py --materias Física Química
    python pipeline/processar_tudo.py --force   # reprocessa mesmo os já prontos
"""
...
DIR_PDFS = PROJETO_ROOT / "pdfs_originais" / "ita_fase1"
...
def ja_processado(ano: int, materia: str) -> bool:
    ...
    prova_id = f"ita_{ano}_fase1{sufixo}"
...
def main():
    ...
    parser.add_argument(
        "--materias", nargs="+", default=ALL_MATERIAS,
        choices=list(SUFIXO.keys()),
        metavar="MATERIA",
        help="Matérias a processar (padrão: Física Química Matemática)"
    )
