"""
processar_2fase.py — Processa todas as provas dissertativas (2ª fase) do ITA.

Na 2ª fase, cada matéria é um PDF separado com 10 questões dissertativas
(1 a 10), sem alternativas A-E e sem gabarito objetivo.

Espera arquivos com o padrão:
    {materia}_{ano}_2f.pdf
ex: fisica_2019_2f.pdf, matematica_2020_2f.pdf, quimica_2025_2f.pdf

Uso:
    python pipeline/processar_2fase.py
    python pipeline/processar_2fase.py --dir pdfs_originais/ita_fase2
    python pipeline/processar_2fase.py --force
    python pipeline/processar_2fase.py --materias Física Matemática
"""
...
DIR_2FASE_DEFAULT = (PROJETO_ROOT / "pdfs_originais" / "ita_fase2").resolve()
...
def descobrir_pdfs(dir_2fase: Path) -> list[tuple[Path, int, str]]:
    """Retorna lista de (pdf_path, ano, materia) ordenada por (ano, materia)."""
    encontrados = []
    for pdf in dir_2fase.glob("*.pdf"):
        m = re.match(r"(fisica|matematica|quimica)_(20\d{2})_2f\.pdf$", pdf.name, re.IGNORECASE)
        if not m:
...
def prova_id_for(ano: int, materia: str) -> str:
    return f"ita_{ano}_fase2{SUFIXO_MATERIA[materia]}"
...
def main():
    ...
    for pdf, ano, materia in pdfs:
        ...
        print(f"\n{'='*60}")
        print(f"  {pid}  ({materia} {ano} — 2ª fase)")
        print(f"{'='*60}")
        ...
        if sucesso:
            ...
            with open(log_dir / "processamento_2fase.log", "a") as f:
                f.write(f"[ERRO] {pid}: pipeline retornou código != 0\n")
            print(f"[ERRO] {pid} falhou — registrado em logs/processamento_2fase.log")
